...
export function createDateTimeTool(
    ...
    return {
        definition: {
            ...
            inputSchema: {
                ...
                properties: {
                    operation: {
                        type: 'string',
                        enum: ['now', 'convert_timezone', 'add_duration', 'difference', 'inspect', 'from_epoch'],
                    ...
                    timeZone: {
                        type: 'string',
                        description: 'IANA 时区，例如 Asia/Shanghai；now、add_duration、inspect、from_epoch 使用。',
                    },
                    targetTimeZone: {
                        type: 'string',
        ...
        async execute(argumentsValue, signal) {
            ...
            try {
                const result = executeOperation(argumentsValue as unknown as DateTimeArguments, now)
                throwIfAborted(signal)
...
function executeOperation(input: DateTimeArguments, now: () => Temporal.Instant): Record<string, unknown> {
    switch (input.operation) {
        ...
        case 'convert_timezone': {
            ...
            const source = parseZonedDateTime(dateTime, input.timeZone)
            return {
                operation: input.operation,
        ...
        case 'inspect': {
            ...
            return {
                operation: input.operation,
                result: snapshot(parseZonedDateTime(dateTime, input.timeZone)),
            }
        }
        case 'from_epoch': {
...
function parseZonedDateTime(value: string, requestedTimeZone?: string): Temporal.ZonedDateTime {
