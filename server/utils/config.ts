...
export const config = {
    ...
    generationDbPath: process.env.GENERATION_DB_PATH || 'server/data/generation.sqlite',
    generationRetentionDays: Math.min(3650, Math.max(1, Math.floor(numberFromEnv('GENERATION_RETENTION_DAYS', 7)))),
    generationCleanupIntervalMs: Math.min(7 * 86_400_000, Math.max(60_000, Math.floor(numberFromEnv('GENERATION_CLEANUP_INTERVAL_MS', 86_400_000)))),
    generationMaxOutputChars: Math.min(2_000_000, Math.max(1000, Math.floor(numberFromEnv('GENERATION_MAX_OUTPUT_CHARS', 200_000)))),
    generationMaxEventsPerRun: Math.min(50_000, Math.max(100, Math.floor(numberFromEnv('GENERATION_MAX_EVENTS_PER_RUN', 5000)))),
    generationDeltaFlushIntervalMs: Math.min(2000, Math.max(50, Math.floor(numberFromEnv('GENERATION_DELTA_FLUSH_INTERVAL_MS', 200)))),
    generationDeltaFlushChars: Math.min(16_384, Math.max(256, Math.floor(numberFromEnv('GENERATION_DELTA_FLUSH_CHARS', 2048)))),
